...
public class UserAuthControllerTests {
    ...
    public void registerSuccessTest() {
        ...
        when(passwordService.decrypt(TEST_PASSWORD)).thenReturn(TEST_PASSWORD);
        when(userService.createUser(TEST_PHONE, TEST_PASSWORD, TEST_NICKNAME)).thenReturn(user);

        response = userAuthController.registerUser(queryObject);

        assertSuccessResponse(response);
        verify(userService, times(1)).createUser(TEST_PHONE, TEST_PASSWORD, TEST_NICKNAME);
    ...
    public void loginPasswordTest() {
        ...
        when(userService.phoneExists(TEST_PHONE)).thenReturn(true);
        when(passwordService.decrypt(TEST_PASSWORD)).thenReturn(TEST_PASSWORD);
        when(userService.validateUser(TEST_PHONE, TEST_PASSWORD)).thenReturn(true);
        when(userService.getUserIdByPhone(TEST_PHONE)).thenReturn(TEST_USER_ID);
    ...
    public void loginPasswordIncorrectTest() {
        ...
        when(userService.phoneExists(TEST_PHONE)).thenReturn(true);
        when(passwordService.decrypt(TEST_PASSWORD)).thenReturn(TEST_PASSWORD);
        when(userService.validateUser(TEST_PHONE, TEST_PASSWORD)).thenReturn(false);
    ...
    public void changePasswordTest() {
        ...
        assertSuccessResponse(response);
        verify(userService, times(1)).updateUserPasswordByPhone(TEST_PHONE, TEST_PASSWORD);
